...
public static class RegexPatterns
{
    ...
    // Post-processing
    public static string MergerStart = @"\[Merger\]\s*Merging formats into\s*""(?<path>.+)"""; 
    ...
    public static string ErrorLine = @"ERROR:\s*(?<message>.+)";

    // YouTube / extractor
    public static string YoutubeDownloadingWebpage = @"\[youtube\]\s*(?<id>[\w-]{11}):\s*Downloading webpage";
    public static string YoutubeDownloadingPlayer = @"\[youtube\]\s*(?<id>[\w-]{11}):\s*Downloading player\s*(?<player>[\w-]+)?";
    public static string InfoFormat = @"\[info\]\s*Downloading format\(s\):\s*(?<formats>.+)";

    // Generic fallback
    public static string GenericLine = @"\[(?<source>[^\]]+)\]\s*(?<content>.+)";
